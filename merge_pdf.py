...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument("-a", "--all", help="merge all pdfs in the directory, and save as *all*")
    return parser
...
def main() -> int:
    ...
    if not args.files:
        if args.all is not None:
            logger.warning(f'merging all pdfs in the directory and saving as {args.all}')
        ...
        logger.warning(f'merging {args.files[:-1]} pdfs in the directory and saving as {args.files[-1]}')
